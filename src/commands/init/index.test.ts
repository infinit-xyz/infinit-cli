  ...
  describe('functionality with npm', () => {
    ...
    test('should initialize INFINIT project successfully with npm', async () => {
      const [exitCode, logs] = await InfinitCLI()
        .setCwd(cwdPath)
        .invoke(['init', '--directory', newProjectPath, '--chain', 'ARB_SEPOLIA', '--module', 'aave-v3', '--ignore-deployer', '--ignore-analytics'])
    ...
    test('should throw error with project directory does not exist', async () => {
      const [exitCode, _logs, errorLogs] = await InfinitCLI()
        .setCwd(cwdPath)
        .invoke(['init', '--directory', 'wrong', '--chain', 'ARB_SEPOLIA', '--module', 'aave_v3', '--ignore-deployer', '--ignore-analytics'])
    ...
    test('should throw error with protocol module is not supported', async () => {
      const [exitCode, _logs, errorLogs] = await InfinitCLI()
        .setCwd(cwdPath)
        .invoke(['init', '--directory', newProjectPath, '--chain', 'ARB_SEPOLIA', '--module', 'abcd', '--ignore-deployer', '--ignore-analytics'])
